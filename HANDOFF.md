...
- 待办：① 用户配置 `backend/.env` 的 LLM_API_KEY 后联调真实问答/测验/讲解（当前无 Key，接口正确返回错误提示）；② 前端 UI 打磨与真实课件数据测试；③ 回填开发文档占位字段（行数/截图/实测数据）；④ 答辩 PPT（含技术栈讲解，用户要求开发完毕后讲一遍"用了什么技术、为什么、为什么比别的好"）。
